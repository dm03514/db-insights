...
import (
	...
	"github.com/dm03514/db-insights/pkg/postgres"
...
func newService(c *cli.Context) (*service.Service, error) {
	...
	switch c.String("db") {
	case "postgres":
		db, err = postgres.New(c.String("connection-string"))
	case "snowflake":
	...
	app := &cli.App{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				Name:  "db",
				Value: "snowflake",
				Usage: "The concrete database to use: 'snowflake|postgres'",
			},
			&cli.PathFlag{
				Name:  "conf",
				Value: "",
